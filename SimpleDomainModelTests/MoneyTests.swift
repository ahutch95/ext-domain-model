...
class MoneyTests: XCTestCase {
    ...
    func testCSC() {
        let response = tenUSD.description
        let response2 = fiveGBP.description
        let response3 = fifteenCAN.description
        
        XCTAssert(response == "USD10")
        XCTAssert(response2 == "GBP5")
        XCTAssert(response3 == "CAN15")
    }
    ...
    func testDoubleExtentions() {
        let tUSD = 10.USD
        let tGBP = 10.GBP
        let fEUR = 5.EUR
        let fCAN = 15.CAN
        
        XCTAssert(tUSD.amount == 10.0)
        XCTAssert(tUSD.currency == "USD")
        XCTAssert(tGBP.amount == 10.0)
        XCTAssert(tGBP.currency == "GBP")
        XCTAssert(fEUR.amount == 5.0)
        XCTAssert(fEUR.currency == "EUR")
        XCTAssert(fCAN.amount == 15.0)
        XCTAssert(fCAN.currency == "CAN")
